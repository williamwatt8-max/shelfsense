  ...
  if (step === 'upload') {
    return (
      <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-6">
        ...
        {preview && (
          <img src={preview} alt="Receipt preview" className="max-w-xs max-h-64 rounded-lg mb-6 border border-gray-700" />
        )}
        ...
        {loading && (
          <p className="text-gray-500 mt-4 text-sm">AI is reading your receipt...</p>
        )}
        <a href="/inventory" className="text-gray-500 hover:text-gray-300 mt-8 text-sm">
          View Inventory
        </a>
      </main>
    )
  ...
  if (step === 'reviewing') {
    return (
      <main className="min-h-screen bg-gray-950 text-white p-6 max-w-2xl mx-auto">
        ...
        <p className="text-gray-400 mb-6">From {retailer} — edit anything that looks wrong</p>
        <div className="space-y-3">
          {items.map((item, index) => (
            <div
              key={item.id}
              className={'bg-gray-900 border rounded-lg p-4 ' + (item.confidence < 0.8 ? 'border-yellow-600' : 'border-gray-800')}
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    const updated = [...items]
                    updated[index].normalized_name = e.target.value
                    setItems(updated)
                  }}
                  className="bg-gray-800 border border-gray-700 rounded px-3 py-1.5 flex-1 text-white"
                />
                {item.confidence < 0.8 && (
                  <span className="text-yellow-500 text-xs">Low confidence</span>
                )}
              </div>
              <div className="flex gap-2 flex-wrap">
                <input
                  ...
                  className="bg-gray-800 border border-gray-700 rounded px-3 py-1.5 w-20 text-white"
                />
                <input
                  type="text"
                  value={item.unit}
                ...
                <select
                  ...
                  onChange={(e) => {
                    const updated = [...items]
                    updated[index].location = e.target.value as StorageLocation
                    setItems(updated)
                  }}
        ...
        <button
          onClick={handleSave}
          className="mt-6 w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-3 rounded-lg text-lg transition-colors"
        >
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-6">
      ...
      <p className="text-gray-400 mb-6">
        {items.filter((i) => i.selected).length} items added to your inventory
      </p>
